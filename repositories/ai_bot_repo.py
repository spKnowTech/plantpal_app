...
def get_chat_history(db: Session, user_id: int) -> list[dict]:
    ...
    try:
        ...
        # We'll need to track welcome messages with session ID too
        ...
        ).order_by(AIResponse.created_at.desc()).limit(1).all()  # Get most recent welcome message
...
def get_current_session_chat_history(db: Session, user_id: int, session_start_time=None) -> list[dict]:
    ...
    try:
        ...
        if not session_start_time:
            ...
            latest_response = db.query(AIResponse).filter(
                AIResponse.user_id == user_id,
                AIResponse.is_permanent == True
        ...
        logs = db.query(AILog).filter(
            ...
            AILog.type == "chat",
            AILog.is_permanent == True,
            AILog.created_at >= session_start_time
        ).order_by(AILog.created_at).all()
...
def get_complete_conversation_history(db: Session, user_id: int) -> str:
    ...
    try:
        ...
        for log in logs:
            # Add user message
            conversation_lines.append(f"User: {log.input_text}")
...
def clear_session_conversation(db: Session, user_id: int):
    """Clear only current session conversation (for fresh start), but keep permanent history."""
    try:
        # This function now only clears temporary session data
        # Permanent conversations are kept for AI context
        pass
    except Exception as e:
        print(f"Error in clear_session_conversation: {str(e)}")
...
def check_duplicate_question(db: Session, user_id: int, user_message: str) -> dict:
    ...
    try:
        ...
        for prev_input in previous_inputs:
            ...
            if normalized_message == normalized_prev:
                # Exact match found
                return {
                    'ai_log_id': prev_input.id,
                    'original_question': prev_input.input_text,
                    'match_type': 'exact'
                }
...
def update_existing_response(db: Session, ai_log_id: int, new_response: str):
    ...
    try:
        ...
        existing_response = db.query(AIResponse).filter(
            AIResponse.ai_log_id == ai_log_id
        ).first()
...
def get_last_user_question(db: Session, user_id: int) -> str:
